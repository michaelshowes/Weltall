...
export function initProject(projectName: string) {
	...
	execSync(
		`git clone --branch main --depth 1 https://github.com/michaelshowes/nextjs-basic.git ${projectName}`
	);
	...
	execSync(
		`cd ${projectDir} && git add . && git commit -m "initial commit" && git checkout -b staging && git checkout -b development`
	);
	...
	console.log(
		blue('[weltall] ') + '\uD83E\uDD18  Done in ' + process.uptime() + 's.\n'
	);
}
